...
app = xw.App()
#  路径问题，更换请注意
workbook = app.books.open('/Users/ze/Desktop/python/zad/23W执行率计算表.xlsx')
...
def main():
    #  路径问题，更换请注意
    time_calculate = pd.read_excel('/Users/ze/Desktop/python/zad/23W执行率计算表.xlsx', sheet_name='时刻')
    ...
    worksheet1.range('Q3').value = time_calculate_list
    workbook.save()
    read_result()
...
def read_result():
    ...
    for flight in list2:
        ...
        if len(index_list) % 3 == 0:
            cell1 = worksheet1.range('F2').offset(row_offset=index_list[0] + 1).value
            cell2 = worksheet1.range('F2').offset(row_offset=index_list[1] + 1).value
            cell3 = worksheet1.range('F2').offset(row_offset=index_list[2] + 1).value
            zhixinglv1 = worksheet1.range('P2').offset(row_offset=index_list[0] + 1).value
            zhixinglv2 = worksheet1.range('P2').offset(row_offset=index_list[1] + 1).value
            zhixinglv3 = worksheet1.range('P2').offset(row_offset=index_list[2]+ 1).value
...
def FOC_main():
    ...
    time_calculate = pd.read_excel('/Users/ze/Desktop/python/zad/23W执行率计算表.xlsx', sheet_name=0)
    # 1 简单的条件筛选：单一条件筛选
    df = df[df["飞行时间"] >= 10]
...
def foc_month_main(month, date_1, dict):
    month = str(month) + "月"
    #  路径问题，更换请注意
    maintable = pd.read_excel('/Users/ze/Desktop/python/zad/23W执行率计算表.xlsx', sheet_name=month)
...
def check():
    # 先检查每一天，然后再检查
    date1 = workbook.sheets['FOC数据汇总'].range('K1').value
    date2 = workbook.sheets['FOC数据汇总'].range('K2').value
